...
headline += "<form method='post' action='inProduc.py'> 商品入庫 : <input type='text' name='inID' required='required' placeholder='在此輸入商品編號'> 數量 : <input type='number' name='inNum' required='required' placeholder='在此輸入入庫數量'>&nbsp<input type='submit' value='執行'></form></div>"
headline += "<form method='post' action='outProduc.py'> 商品出庫 : <input type='n' name='outID' required='required' placeholder='在此輸入商品編號'> 數量 : <input type='number' name='outNum' required='required' placeholder='在此輸入出庫數量'>&nbsp<input type='submit' value='執行'></form></div>"
headline += "<hr>"
dialog =""#初始化，避免報錯
#獲取商場資訊，並建置表格
